...
def cleanup_on_exit():
    """Cleanup mechanism that runs when the program exits.
    - Wipes all folders inside the HTML folder
    - Deletes all CSV files in the workspace
    - Deletes wait.json file
    """
    print("\n🧹 Starting cleanup process...")

    # 1. Wipe all folders inside the HTML folder
    html_path = Path(HTML_FOLDER)
    if html_path.exists() and html_path.is_dir():
        try:
            # Get all subdirectories in html folder
            for item in html_path.iterdir():
                if item.is_dir():
                    shutil.rmtree(item)
                    print(f"   ✅ Deleted folder: {item.name}")
        except Exception as e:
            print(f"   ⚠️  Error cleaning HTML folders: {e}")

    # 2. Delete all CSV files in the workspace
    current_dir = Path(".")
    try:
        for csv_file in current_dir.glob("*.csv"):
            try:
                csv_file.unlink()
                print(f"   ✅ Deleted CSV file: {csv_file.name}")
            except Exception as e:
                print(f"   ⚠️  Could not delete {csv_file.name}: {e}")
    except Exception as e:
        print(f"   ⚠️  Error cleaning CSV files: {e}")

    # 3. Delete wait.json file
    wait_file_path = Path(WAIT_FILE)
    if wait_file_path.exists():
        try:
            wait_file_path.unlink()
            print(f"   ✅ Deleted file: {WAIT_FILE}")
        except Exception as e:
            print(f"   ⚠️  Could not delete {WAIT_FILE}: {e}")
...
def write_conf(configs):
    """Write the configuration dictionary back to the Python config file."""
    import pprint

    content = f"""# Configuration file for Handshake automation
...
def read_conf():
    ...
    # Load the conf.py module dynamically
    ...
    # Convert module variables to dictionary format for backward compatibility
    configs = {
        "valid": conf_module.VALID,
        "url": sanitize_url(conf_module.URL),
        "resume": conf_module.RESUME,
        "cover": conf_module.COVER,
        "transcript": conf_module.TRANSCRIPT,
        "other": getattr(conf_module, 'OTHER', None),
        "date": conf_module.DATE,
        "job_keywords": getattr(conf_module, 'JOB_KEYWORDS', []),
        "skip_keywords": getattr(conf_module, 'SKIP_KEYWORDS', []),
        "cookies": conf_module.COOKIES
    }
